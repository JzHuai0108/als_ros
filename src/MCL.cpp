...
void MCL::updateParticlesByMotionModel(void) {
...
template std::vector<float> MCL::getResidualErrors<float>(Pose pose);
template std::vector<double> MCL::getResidualErrors<double>(Pose pose);

void MCL::printResult(void) {
...
void MCL::publishROSMessages(void) {
    // pose
...
void MCL::scanCB(const sensor_msgs::LaserScan::ConstPtr &msg) {
    if (canUpdateScan_)
        scan_ = *msg;
    if (!gotScan_)
        gotScan_ = true;
...
void MCL::mapCB(const nav_msgs::OccupancyGrid::ConstPtr &msg) {
...
void MCL::initialPoseCB(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg) {
    tf::Quaternion q(msg->pose.pose.orientation.x, 
        msg->pose.pose.orientation.y, 
        msg->pose.pose.orientation.z,
        msg->pose.pose.orientation.w);
...
void MCL::glSampledPosesCB(const geometry_msgs::PoseArray::ConstPtr &msg) {
...
double MCL::calculateLikelihoodFieldModel(Pose pose, double range, double rangeAngle) {
...
double MCL::calculateClassConditionalMeasurementModel(Pose pose, double range, double rangeAngle) {
    if (range <= scan_.range_min || scan_.range_max <= range)
        return measurementModelInvalidScan_;

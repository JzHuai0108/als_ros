...
GLPoseSampler::GLPoseSampler(void):
    nh_("~"),
    mapName_("/map"),
    scanName_("/scan"),
    odomName_("/odom"),
...
void GLPoseSampler::setMapInfo(nav_msgs::OccupancyGrid map) {
...
std::vector<Keypoint> detectKeypoints(nav_msgs::OccupancyGrid map, cv::Mat distMap, Pose &mapOrigin_,
        double keypointsMinDistFromMap_, double gradientSquareTH_) {
...
std::vector<SDFOrientationFeature> calculateFeatures(cv::Mat distMap, std::vector<Keypoint> keypoints, 
        double sdfFeatureWindowSize_, double mapResolution_) {
...
visualization_msgs::Marker GLPoseSampler::makeSDFKeypointsMarker(std::vector<Keypoint> keypoints, std::string frame) {
...
void GLPoseSampler::writeMapAndKeypoints(nav_msgs::OccupancyGrid map, std::vector<Keypoint> keypoints) {
    FILE *fp;
...
void GLPoseSampler::mapCB(const nav_msgs::OccupancyGrid::ConstPtr &msg) {
    setMapInfo(*msg);
    cv::Mat distMap = buildDistanceFieldMap(*msg);
    cv::GaussianBlur(distMap, distMap, cv::Size(5, 5), 5);
    sdfKeypoints_ = detectKeypoints(*msg, distMap, mapOrigin_, keypointsMinDistFromMap_, gradientSquareTH_);
    sdfOrientationFeatures_ = calculateFeatures(distMap, sdfKeypoints_, sdfFeatureWindowSize_, mapResolution_);
...
nav_msgs::OccupancyGrid GLPoseSampler::buildLocalMap(void) {
...
geometry_msgs::PoseArray GLPoseSampler::generatePoses(Pose currentOdomPose, std::vector<Keypoint> localSDFKeypoints,
